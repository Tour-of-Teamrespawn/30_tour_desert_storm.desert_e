...
A2S_taskHintLocal =
{
	...
	if ([_fncName, _this, ["STRING", "STRING"]] call A2S_friendlyError_paramsMatch) then
	{
		if (!isDedicated) then
		{
			private ["_taskName", "_hint", "_params"];
			_taskName = _this select 0;
			_hint = _this select 1;
			if ((!isNil {A2S_missionCore getVariable ("A2S_task_" + _taskName)})&&(toLower _hint in ["tasksucceeded","taskfailed","taskcanceled","taskcreated","taskassigned", "taskupdated"])) then
			{
				if (!isNil _taskName) then
				{
					[_hint ,["",(call ((A2S_missionCore getVariable ("A2S_task_" + _taskName)) select 3))]] call BIS_fnc_showNotification;
				};
			}
			else
			{
...
A2S_taskHint =
{
	...
	if ([_fncName, _this, ["STRING", "STRING"]] call A2S_friendlyError_paramsMatch) then
	{
		private ["_taskName", "_hint", "_msg"];
		_taskName = _this select 0;
		_hint = _this select 1;
		
		if ((!isNil {A2S_missionCore getVariable ("A2S_task_" + _taskName)})&&(toLower _hint in ["tasksucceeded", "taskfailed", "taskcanceled", "taskcreated", "taskassigned", "taskupdated"])) then
		{
			[_this ,"A2S_taskHintLocal"] call BIS_fnc_MP;
		}
		else
		{
			[format ['ERROR IN %1!|    Task "%2" does not exist', _fncName, _taskName]] call A2S_friendlyError_alert;
